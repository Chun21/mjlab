"""Minimal Reactive Soccer PPO scaffold."""
...
class ReactiveSoccerPPO:
  """Minimal but learnable PPO implementation for reactive soccer."""
  ...
  @classmethod
  def construct_algorithm(cls, device: str = 'cpu', **kwargs: Any) -> 'ReactiveSoccerPPO':
  ...
  def process_env_step(
    ...
    if dones is None:
      dones = torch.zeros(goal_rewards.shape[0], device=goal_rewards.device, dtype=torch.long)
    ...
    self.storage.add(
      ...
      goal_rewards=goal_rewards.unsqueeze(-1) if goal_rewards.ndim == 1 else goal_rewards,
  ...
  def update(self) -> dict[str, float]:
    if self.actor_critic is None or self.optimizer is None or self.storage is None or self.storage.step == 0:
      return {
    ...
    dist = self._dist(outputs['actions_mean'])
    log_probs = dist.log_prob(batch['actions']).sum(dim=-1, keepdim=True)

    total_adv = self.combine_advantages(
      batch['goal_advantages'],
      batch['aux_advantages'],
      w_goal=self.w_goal,
      w_aux=self.w_aux,
    )
    total_adv = (total_adv - total_adv.mean()) / total_adv.std().clamp(min=1.0e-6)
    ...
    goal_value_loss = F.mse_loss(outputs['goal_value'], batch['goal_returns'])
    aux_value_loss = F.mse_loss(outputs['aux_value'], batch['aux_returns'])
    reconstruction_loss = F.mse_loss(outputs['reconstruction'], batch['reconstruction_targets'])
    ...
    return {
      'surrogate': float(surrogate_loss.detach().item()),
      'goal_value': float(goal_value_loss.detach().item()),
      'aux_value': float(aux_value_loss.detach().item()),
      'reconstruction': float(reconstruction_loss.detach().item()),
      'symmetry': float(symmetry_loss.detach().item()),
    }

  def save(self) -> dict[str, Any]:
    state: dict[str, Any] = {'device': self.device}
    if self.actor_critic is not None:
  ...
  def get_inference_policy(self) -> Callable[[dict[str, torch.Tensor]], dict[str, torch.Tensor]]:
